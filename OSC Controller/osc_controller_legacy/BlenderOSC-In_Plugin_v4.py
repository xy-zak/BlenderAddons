bl_info = {
    ...
    "description": "Control object properties using OSC messages over LAN",
    "category": "Object",
}
...
def check_pythonosc():
    ...
    try:
        ...
        try:
            ...
            pythonosc_path = "Installed (path unknown)"
        return True
    except ImportError:
        pythonosc_available = False
        pythonosc_path = "Not installed"
...
class OSC_PT_MainPanel(Panel):
    ...
    def draw(self, context):
        ...
        if pythonosc_available:
            row.label(text="python-osc: Installed", icon='CHECKMARK')
        else:
            row.label(text="python-osc: Not Installed", icon='X')
            row = box.row()
